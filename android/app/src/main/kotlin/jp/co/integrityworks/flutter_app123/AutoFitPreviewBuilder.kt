...
class AutoFitPreviewBuilder private constructor(
    ...
    init {
        ...
        useCase.setOnPreviewOutputUpdateListener(Preview.OnPreviewOutputUpdateListener {
            val viewFinderUserCase =
                    viewFinderRef.get() ?: return@OnPreviewOutputUpdateListener
            Log.d(TAG, "Preview output changed. " +
                    "Size: ${it.textureSize}. Rotation: ${it.rotationDegrees}")

            // To update the SurfaceTexture, we have to remove it and re-add it
            val parent = viewFinderUserCase.parent as ViewGroup
            parent.removeView(viewFinderUserCase)
            parent.addView(viewFinderUserCase, 0)

            // Update internal texture
            viewFinderUserCase.surfaceTexture = it.surfaceTexture

            // Apply relevant transformations
            bufferRotation = it.rotationDegrees
            val rotation = getDisplaySurfaceRotation(viewFinderUserCase.display)
            updateTransform(viewFinderUserCase, rotation, it.textureSize, viewFinderDimens)
        })
